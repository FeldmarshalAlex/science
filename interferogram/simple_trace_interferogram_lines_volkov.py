...
def trace_line(brightness, start_point):
    ...
    def volkov_trace_step(x, y, old_x, old_y):
        ...
        for direction in range(8):
            ...
            dr = (dx**2 + dy**2)**0.5
            derivative = (brightness[x + dx][y + dy] - brightness[x][y]) / dr
            for k in range(8):
        ...
        direction = max_gradient_direction + 2
    ...
    while (0 + step_distance <= x < len(brightness[0]) - step_distance and
           0 + step_distance <= y < len(brightness) - step_distance):
        line.append((x, y))
